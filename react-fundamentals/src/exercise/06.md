...
- when using useRef it allows you to reference a value thats not needed during
  rendering. The input should have a property set to ref that uses the useRef
  value. This way useRef and the input element are directly connected.
- can use useState to change the form to a controlled form, good if theres
  additional logic you need performed, ,otherwise the browser will handle the
  input.
