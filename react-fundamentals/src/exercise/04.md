...
- By using the React API it creates the same element but as a component, which
  is important when creating and using custom components.
- Capitilize the React Components to allow the compiler to reference the
  functions instead of a string version.
- PropTypes are useful for runtime validation in development, you normally would
  want to use TypeScript instead for type checking.
- PropTypes dont check if properties are required, can use props-types
  .isRequired to check and run even when a property is not passed in.
